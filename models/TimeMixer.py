...
class PastDecomposableMixing(nn.Module):
    ...
    def forward(self, x_list):
        ...
        # print("season_list", len(season_list))
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.pdm_blocks = nn.ModuleList([PastDecomposableMixing(configs) # 包含多层PDM模块的列表，深度为configs.e_layers
    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        ...
        # B,T,C -> B,C,T，调整数据维度，以便执行下采样操作，分别是批量大小、时间步（时间序列长度）、通道数量（特征/变量数量）
        x_enc = x_enc.permute(0, 2, 1)  # 调整数据输入的维度 B,C,T
        ...
        x_enc_sampling_list.append(x_enc.permute(0, 2, 1)) # B,T,C，这里存放的是原始输入序列（原始尺度）
        x_mark_sampling_list.append(x_mark_enc)

        # 对每一层执行下采样，将每层的采样结果添加到之前创建的两个列表中
        # 主要用于生成不同分辨率的多尺度输入，便于捕捉数据的不同时间尺度特征
        # print("self.configs.down_sampling_window", self.configs.down_sampling_window)
        # print("self.configs.down_sampling_layers", self.configs.down_sampling_layers)
        # self.configs.down_sampling_layers = 5
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if x_mark_enc is not None:
            for i, x, x_mark in zip(range(len(x_enc)), x_enc, x_mark_enc): # 遍历各个尺度
                B, T, N = x.size()
        ...
        for i in range(self.layer): # 对每一层进行循环（PDM Block的数量）
...
# if __name__ == '__main__':  # 单元测试 看自己的数据shape是否能跑通模型
#     config = None  # args
#     model = Model(config)  # TODO
#     x_enc = torch.randn(31, 1440, 2880)
#     x_dec = torch.randn()
#
